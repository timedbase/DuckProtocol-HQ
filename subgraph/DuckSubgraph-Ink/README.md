...
Subgraph for DuckProtocol on Ink (57073), deployed to Goldsky as `duck-subgraph-ink`.
...
| DuckHookV4 | `0x5a05a1f0A101237D8c350EFfA54f4b3c9bc142cc` |
...
All start at block 55728270. `DuckToken`, `DuckVault`, `DuckTokenGovernor` and
...
Prices are recorded both in quote units per token and in USD. Stablecoins (USDT0, USDC (bridged) and USDG) count as
exactly $1. ETH/USD comes from the WETH/USDT0 and WETH/USDG v3 pools, weighted by their in-range liquidity. Every other curated quote token is priced from its deepest
